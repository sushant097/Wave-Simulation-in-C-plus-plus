...
class Wave_Simul
{
    ...
    public:
        Wave_Simul()
        {
            ...
            amp=0;
            freq=0;
            bak_color_choice=0; // black
            font_color_choice = 15; //white
        ...
        void check();
        void printText(int);
        void printAmp(double,double,int);
...
void Wave_Simul::initialize()
{
    gd=DETECT,gmode=0;
            ...
            //bak_color_choice = 15; // black
            initwindow(1250,700,"Wave-Simulation");
           setbkcolor(bak_color_choice);
           setcolor(font_color_choice);
}
void Wave_Simul::printAmp(double yP,double fq,int type)
{
    char msg[10];
    settextstyle(2,HORIZ_DIR, 0);
    x= x_left_border();
    sprintf(msg,"-%d",(int)yP);
//    yP= amp * sin(RADIANS(*fq));
    outtextxy(X0,(int)yP,msg);
...
void Wave_Simul::drawSineWave()
{
  ...
  /* Draw pixel at x,y */
  putpixel(X(x), Y(y), font_color_choice);
...
void Wave_Simul::addWave()
{
   x=0;y=0;
    ...
    if(camp>samp)
    {
        ...
        y = (camp* (cos(RADIANS(x*cfreq)))) + (samp*( sin(RADIANS(x*sfreq))));
...
int main()
{
    ...
    Wave_Simul wave;
        ...
        cleardevice();
        system("COLOR 4F"); //backgroundColor+FontColor
    //clrscr();
    int choice;
